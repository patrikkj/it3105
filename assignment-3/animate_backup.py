...
class AnimateMC():
    def __init__(self, actor, env, n_steps=1000, x_range=[-1.2,0.6]):
        ...
        #self.ax.autoscale_on(True)
        #TESTING
        #self.fig = plt.figure(figsize=(15,10))
        #self.ax = self.fig.add_subplot(111, aspect=2, autoscale_on=False, xlim=(x_range[0], x_range[1]), ylim=(-1.2, 1.2))
        ...
        self.im = self.ax.imshow(self.car_img, interpolation='none',
                    origin='upper', aspect = "2",
                    extent=[-0.1, 0.1, -0.04, 0.04], clip_on=True)
